...
public class ItemPickUp : NetworkBehaviour
{
    ...
    private async void OnTriggerEnter2D(Collider2D other)
    {
        ...
        await ServerInventoryManager.Instance.AddItemToPlayerInventory(playerData, item.ItemCode);

        Debug.Log($"[ItemPickUp] Item pickup processing completed for item {item.ItemCode}");

        Debug.Log($"Destroying GameObject: {other.gameObject.name} at position {other.transform.position}");
        Debug.Log($"GameObject active before destroy: {other.gameObject.activeInHierarchy}");
        NetworkServer.Destroy(other.gameObject);
        Debug.Log("Destroy() called successfully");
